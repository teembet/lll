<template>
  ...
  <option v-for="(item,index) in networks" :key="index" :value="item.id">{{item.name}}</option>
</select>
          </div>
          <div class="col">
            <label class="formlabel" for="formGroupExampleInput"
...
export default {
  ...
  methods:{
    ...
    async runAirtel(formData,local_token){
      try {
        const response = await axios().post(process.env.BASE_URL  + 'paysure/transaction/airtimevtu/airtel',formData,  {
                headers: {
   ...
   async runMtn(formData,local_token){
    ...
    sendRequest(){
       ...
       const formData ={
            ...
            channel: "Mobile",
            encryptedData: "AUNueSmYRPpzYT6U/EaWOvILo8b+RVrsxA/B/RWdv1Q=",
            extraFields: {
            imei: "",
            plainText: "amount=100&beneficiaryNumber=08109985849&merchantCode=837E741B40D&channel=Mobile"
            },
